...
build_and_append_tenure <- function(scales_variables_modules, scales_sequences, crs,
  ...
  data <- data[data$ID %in% c(scales_variables_modules$scales$CT$ID, paste0("Secteur ", 1:6)), ]

  data$ID[data$ID == "Secteur 1"] <- scales_variables_modules$scales$secteur$ID[
    scales_variables_modules$scales$secteur$name == "Duvernay, Saint-Vincent-de-Paul, Saint-François"
  ]
  data$ID[data$ID == "Secteur 2"] <- scales_variables_modules$scales$secteur$ID[
    scales_variables_modules$scales$secteur$name == "Laval-des-Rapides, Pont-Viau"
  ]
  data$ID[data$ID == "Secteur 3"] <- scales_variables_modules$scales$secteur$ID[
    scales_variables_modules$scales$secteur$name == "Chomedey"
  ]
  data$ID[data$ID == "Secteur 4"] <- scales_variables_modules$scales$secteur$ID[
    scales_variables_modules$scales$secteur$name == "Laval-Ouest, Sainte-Dorothée, Laval-sur-le-Lac"
  ]
  data$ID[data$ID == "Secteur 5"] <- scales_variables_modules$scales$secteur$ID[
    scales_variables_modules$scales$secteur$name == "Sainte-Rose, Fabreville"
  ]
  data$ID[data$ID == "Secteur 6"] <- scales_variables_modules$scales$secteur$ID[
    scales_variables_modules$scales$secteur$name == "Vimont, Auteuil"
  ]
  ...
  if (length(only_scales_exc) != 0) {
    ...
    data_interpolated <-
      interpolate_from_census_geo(
        ...
        only_scales = only_scales_exc[!only_scales_exc %in% c("secteur", "CT")],
        crs = crs,
        overwrite = overwrite,
        time_regex = "_\\d{4}$",
        inst_prefix = inst_prefix
    ...
    # Other scales
    data_interpolated$scales <- c(data_interpolated$scales[!names(data_interpolated$scales) %in% c("secteur", "CT")],
                                  scales_variables_modules$scales[c("secteur", "CT")])

    # Merge the data to the CT centraide scale
    names(data)[1] <- "ID"
    data_interpolated$scales$CT <-
      dplyr::left_join(data_interpolated$scales$CT, data, by = "ID")

    data_interpolated$scales$secteur <-
      dplyr::left_join(data_interpolated$scales$secteur, data, by = "ID")

    # Rewrite NO interpolation for Centraide region
    data_interpolated$interpolated_ref <-
      rbind(data_interpolated$interpolated_ref,
            tibble::tibble(scale = c("CT", "secteur"),
                           interpolated_from = c("FALSE", "FALSE")))

    # Adding CT and Centraide to available scales
    data_interpolated$avail_scale <- c("CT", data_interpolated$avail_scale)
  ...
  only_scales <- only_scales[!only_scales %in% c("secteur", "CT")]
  interpolated_df <- tibble::tibble(scale = only_scales, interpolated_from = "CT")
  interpolated_df <- rbind(interpolated_df,
                           tibble::tibble(scale = c("secteur", "CT"),
                                          interpolated_from = c(FALSE, FALSE)))
  only_scales <- unique(c(only_scales, "CT", "secteur"))
